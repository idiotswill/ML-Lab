...
from ml_lab.baselines.phase_a_provider import run_phase_a_local_provider_dev_baseline
...
DEV_PROVIDER_RECEIPT_NAME = "phase-a-expanded-dev-provider-receipt.json"


def run_expanded_phase_a_dev_provider(
    *,
    workspace_path: Path,
    ...
    if len(result.summaries) != 1 or result.summaries[0].split is not DatasetSplit.DEV:
        raise RuntimeError("Local provider DEV probe did not produce exactly one DEV summary")
    summary = result.summaries[0]

    evaluation = EvaluationService(workspace)
    if evaluation.case_count(result.experiment.id, DatasetSplit.TEST) != 0:
        raise RuntimeError("Local provider DEV probe created TEST evidence")
    if evaluation.case_count(result.experiment.id, DatasetSplit.REDTEAM) != 0:
        raise RuntimeError("Local provider DEV probe created REDTEAM evidence")
    ...
    (workspace.root / DEV_PROVIDER_RECEIPT_NAME).write_bytes(
