...
from ml_lab.trainers.phase_a_candidate_sparse import train_phase_a_candidate_sparse
...
def task_phase_a_candidate_sparse_train(
    spec: JobSpec,
    staging: Path,
) -> dict[str, object]:
    train_paths, _dev_paths = _training_paths(spec.payload)
        ...
        for record in read_jsonl_records(train_paths):
            ...
            if train_examples == 1 or train_examples - last_reported >= 100:
                ...
                _emit(
                    ...
                    progress=min(0.9, 0.05 + train_examples / 50_000),
                    message=(
                        "Trained candidate-relative Phase A scorer on "
    ...
    return {
        "ok": True,
        "trainer_id": "builtin.phase_a_candidate_sparse.v1",
        "experiment_id": spec.payload.get("experiment_id"),
        "train_examples": train_examples,
        "feature_dim": feature_dim,
        "alpha": alpha,
        "outputs": [
            {
                ...
                "path": "model.json",
                "media_type": (
                    "application/vnd.ml-lab.phase-a-candidate-relative-sparse+json"
                ),
            }
        ],
    }
...
TASKS: dict[str, Task] = {
    ...
    "trainer.phase_a_candidate_sparse.v1": task_phase_a_candidate_sparse_train,
}
