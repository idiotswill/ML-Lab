...
_MIN_FAMILY_POSTERIOR = 0.70
_MIN_SLOT_POSTERIOR = 0.80


class BoundedPredictionError(RuntimeError):
    """Raised when the current authority envelope does not permit a safe abstention."""


@dataclass(frozen=True, slots=True)
class PhaseASparseModel:
    format_version: int
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "slot_models": {
                ...
                for family, family_models in sorted(self.slot_models.items())
            },
        }

    @classmethod
    def from_dict(cls, value: Mapping[str, object]) -> PhaseASparseModel:
        if value.get("kind") != "phase-a-bounded-sparse-v1":
            raise ValueError("Unsupported Phase A sparse model kind")
        decision = _model_from_object(value.get("decision_model"), "decision_model")
    ...
    @classmethod
    def load(cls, path: Path) -> PhaseASparseModel:
        decoded = json.loads(path.read_text(encoding="utf-8"))
        if not isinstance(decoded, dict):
            raise ValueError("Phase A sparse model must be a JSON object")
        return cls.from_dict({str(key): item for key, item in decoded.items()})


def train_phase_a_sparse(
    ...
    *,
    ...
    for record in records:
        ...
        if not proposal_check.accepted:
            raise ValueError(
                "Phase A training label violates its exported authority envelope: "
                f"{proposal_check.error_code}"
            )
    ...
    if examples == 0:
        raise ValueError("Phase A training data is empty")
    family_model = family_builder.finish() if family_builder.example_count else None
    return PhaseASparseModel(
        format_version=1,
        training_examples=examples,
        decision_model=decision_builder.finish(),
        family_model=family_model,
        slot_models={
            family: {slot: builder.finish() for slot, builder in builders.items()}
            for family, builders in slot_builders.items()
        },
    )


def predict_phase_a_sparse(
    ...
    adapter.validate_exported_request(request)
    declaration = _declaration(request)
    permitted = _string_values(request.get("permitted_decisions"), "permitted_decisions")
    ...
    proposal: dict[str, object] = {
        ...
        "entity_keys": [],
        "slots": supplied,
        "reason_code": "ML_LAB_BOUNDED_SPARSE",
        "facts_used": [],
        "question": None,
        "missing_slots": [],
        "candidate_keys": [],
    ...
    if not check.accepted:
        raise BoundedPredictionError(
            f"Bounded sparse assembly failed adapter validation: {check.error_code}"
        )
    return proposal
...
def _ask_player(
    ...
    *,
    ...
    if "ASK_PLAYER" not in permitted:
        raise BoundedPredictionError(
            "The current request does not permit ASK_PLAYER, so the scorer cannot abstain safely."
        )
    missing = list(dict.fromkeys(missing_slots))
    if not missing:
    ...
    proposal: dict[str, object] = {
        ...
        "reason_code": "ML_LAB_SPARSE_ABSTAIN",
        "facts_used": [],
    ...
    if not check.accepted:
        raise BoundedPredictionError(
            f"Bounded sparse abstention failed adapter validation: {check.error_code}"
        )
    return proposal
...
def _choose_bounded(
    ...
    *,
    ...
    scores = {
        label: score for label, score in model.score(text).items() if label in allowed_set
    }
    if not scores:
        return None
...
def _model_from_object(value: object, field: str) -> SparseNBModel:
    ...
    return SparseNBModel.from_dict({str(key): item for key, item in value.items()})
